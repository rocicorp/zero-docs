...
const allPostsQueryDef = defineQuery(
  () => zql.post
)
```
...
<Note emoji="😈" heading="Queries don't have to be pure" slug="purity">
It's most common to compute the ZQL expression using only the parameters. But it's not *required*.
...
You can optionally pass a *validator* to `defineQuery` as the first argument:
...
<Note type="warning" emoji="⚠️" heading="Use `defineQueries` at top level only">
...
```tsx
const results = await queries.issues.byPriority('high')
  .run({type: 'complete'})
```
...
```tsx
const [issues, issuesResult] = useQuery(queries.issues.inbox());
