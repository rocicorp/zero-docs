...
Permissions are expressed using ZQL queries and are run automatically as part of every read and write operation.

Because permissions are ZQL, and not some other special rule language, they are quite flexible and can easily express concepts like “allow if in a group that has access”.

## Define Permissions

Permissions are defined in [`schema.ts`](/docs/zero-schema) using the `definePermissions` function. The passed callback returns an object which can have a _policy_ for each table in your schema.

Here's a simple example of returning a policy for an `issue` table that restricts deletes to the issue's creator:
...
<Note type="danger">
If a policy is not defined for a table or operation, Zero currently ***allows*** the operation. This is not a very safe default, but it is productive. We will likely flip this default in the future.
</Note>
...
See [hello-zero](https://github.com/rocicorp/hello-zero/blob/main/schema.ts) for a simple example of write auth and [zbugs](https://github.com/rocicorp/mono/blob/main/apps/zbugs/schema.ts#L16) for a much more involved one.