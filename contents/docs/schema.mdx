...
<Note
  emoji="🧑‍💻"
  heading="Not seeing your generator?"
  slug="contributing"
>
...
Use `unique` to declare a unique index or constraint that already exists in the database. Pass every column for a compound unique key, and call `unique` once for each index:
...
This declaration lets Zero's type system recognize queries that constrain at most one row, including [scalar subqueries](/docs/zql#scalar-subqueries). It does not create or enforce a database constraint, so it must match a real unique index or constraint upstream.
...
<Note
  slug="chain-limit"
  heading="Only two levels of chaining are supported"
>
...
<Note
  type="note"
  heading="Test on staging first"
  slug="test-on-staging"
>
...
<Note
  type="warning"
  heading="Custom publications + Supabase"
  slug="supabase-warning"
>
  If you use a [custom
...
<Note type="warning" emoji="👨‍🏫" slug="incorrect-deployment-order" heading="Incorrect deployment order will cause downtime">
...
<Note heading="Handling old clients">
...
<Note
  type="warning"
  heading="Wait for backfill before deploying code that uses the new data"
  slug="wait-for-backfill"
>
  During backfill, the new column is hidden from the
  replica. If you deploy app or API code that queries the
  column before backfill completes, those queries can fail
  even though the column exists in Postgres.
</Note>

### Monitoring Backfill Progress

You can detect backfill status several ways:

- **Cloud Zero**: Backfill progress is displayed directly in the dashboard.
- **ChangeDB**: Query the `backfilling` table in the ChangeDB. When any rows are returned, Zero is still backfilling:

  ```sql
  SELECT * FROM "<app>/cdc"."backfilling" WHERE "schema" = 'public';
  ```

- **Client observation**: Observe rows synced to a client. After backfill completes, the new column is visible on each row's JavaScript value, regardless of whether the column is present in `schema.ts`.
