...
>
  ...
  cases on the client, operations can go to local storage
  (IndexedDB or SQLite).
</Note>
...
{/* TODO update to create and register the dbProvider with DefaultTypes */}
...
{/* TODO: sendEmailToSubscribers should not be in a defineMutator? */}
...
{/* TODO: this needs to be updated with the latest handleMutateRequest */}
...
app.post('/api/zero/mutate', async c => {
  ...
  // Create mutators dynamically, passing task list.
  const mutators = createMutators(asyncTasks)

  // Run mutators as normal.
  return await c.json(
    await handleMutateRequest(
      transact =>
