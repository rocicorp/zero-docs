...
- Using `order by` and `limit` in many-to-many relationships now throws an error. It didn't work before but did the wrong thing silently. Now it throws a runtime error. See [docs](/docs/zql#junction-relationships), [bug](https://bugs.rocicorp.dev/issue/3527).
