...
function generateLLMsFile() {
  ...
  try {
    ...
    console.log(
      `Processed ${processedFromToc} files from TOC, now checking for linked files...`,
    );
    ...
    console.log(
      `Successfully generated llms.txt with content from ${processedFiles.size} files:`,
    );
    console.log(`- ${processedFromToc} files from TOC`);
    console.log(
      `- ${processedFiles.size - processedFromToc} additional linked files`,
    );
  } catch (error) {
