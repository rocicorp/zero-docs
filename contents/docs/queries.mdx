...
<Note type="warning" emoji="⚠️" heading="Use `defineQueries` only at top level">
...
// With preload():
queries.user.byID('id', userId).preload(
  {ttl: '5m'})

// With run():
const user = await queries.user.byID('id', userId).run(
  {ttl: '5m'})

// With materialize():
const view = queries.user.byID('id', userId).materialize(
  {ttl: '5m'})
```
