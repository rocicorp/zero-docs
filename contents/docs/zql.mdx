...
Do not directly modify JavaScript values returned from ZQL queries.

ZQL caches values across queries to improve performance and reduce re-renders in frameworks like React and Solid. If you modify a value returned from ZQL, you will modify it everywhere it is used. This can lead to subtle bugs.
...
<Note
  type="warning"
  slug="junction-relationships"
  heading="Order and limit not supported in junction relationships"
>
...
<Note type="note" slug="ilike-collation" heading="ILIKE and collation">

Zero can evaluate an `ILIKE` filter in three places: in-memory on the client, in ZQLite against the replica, or as SQL in Postgres.

ZQLite and the in-memory query engine use Unicode-aware lowercasing before matching. This means common non-ASCII case variants, such as `MÜLLER` and `müller`, match in both.

If you need non-ASCII text in PG to work with `ILIKE`, use a Unicode-aware collation for those text columns, such as the database's default UTF-8 locale.
...
To use scalar subqueries, add a unique index in Postgres and [declare it as unique in the Zero schema](/docs/schema#unique-keys), then add `{scalar: true}` to your
`whereExists` call:
...
Scalar subqueries only work when the subquery is guaranteed to return at most one row. The callback must constrain every column of a primary key or a key [declared with `.unique(...)`](/docs/schema#unique-keys) using equality. Compound keys require an equality constraint for every column.
They also only support one-hop relationships, not junctions.
