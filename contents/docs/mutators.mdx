...
The only required argument is a `MutatorFn`, which receives a `tx` that can used along with the [CRUD API](crud) to write data:
...
<Note emoji="✋" type="warning" heading="Always await operations in mutators">
  Mutators almost always complete in the same frame on the client, within milliseconds.
  
  The reason they are marked `async` is because on the server, reading
  from the `tx` object goes over the network to Postgres. Also, in edge cases on
  the client, operations can go to local storage (IndexedDB or SQLite).
</Note>

### Arguments

The `MutatorFn` can take a single `args` parameter of any type, and can use arbitrary logic to perform the mutation:

```ts
import {defineMutator} from '@rocicorp/zero'
import {crud} from 'schema.ts'

const initStats = defineMutator<{issueCount: number}>(
...
<Note emoji="😈" heading="Mutators don't have to be pure" slug="purity">
...
```ts
import {defineMutator} from '@rocicorp/zero'
import {crud, zql} from 'schema.ts'

const updateIssue = defineMutator<{id: string, title: string}>(
  async ({tx, args: {id, title}}) => {
...
```ts
const createIssue = defineMutator<{id: string, title: string}>(
...
export const mutators = defineMutators({
  issue: {
    update: defineMutator<{id: string, title: string}>(
      async ({tx, args: {id, title}}) => {
        await tx.mutate(
          crud.issue.update({
            id,
...
```ts
console.log(mutators.issue.update.mutatorName) // "issue.update"
```

### Validators

You can optionally pass a *validator* to `defineMutator` as the first argument:
...
<Note type="warning" emoji="⚠️" heading="Always use validation in production">
Although validators are *technically* optional, you should consider them required.

This is because Zero mutators run on both the client and [on your server](#server-setup). In the server case, the parameters come from the client and are untrusted. These inputs are validated using the validator you provide.
</Note>
  ...
  posts: {
    ...
    update: defineMutator(
      ...
      async ({tx, context: {userID}, args: {id, title}}) => {
        ...
        await tx.mutate(
          crud.post.update({id, title, authorID: userID})
        )
      },
    ),
  }
...
<Note type="warning" emoji="⚠️" heading="Use `defineMutators` at top level only">
...
<Note emoji="🪖" heading="Knowing is half the battle" slug="why-register">
...
The `@rocicorp/zero` package provides the `handleMutateRequest` and `mustGetMutator` functions to make it easy to implement the endpoint.
...
Zero includes several built-in database adapters. You can also easily create your own. See [ZQL on the Server](server-zql) for more information.
</Note>
...
If Zero receives any response from the mutate endpoint other than HTTP 200, 401, or 403, it will disconnect and enter the [error state](TODO).
...
export const serverMutators = defineMutators(sharedMutators, {
  posts: {
    ...
    update: defineMutator(
      ...
      async (
        {tx, ctx: {userID}, args: {id, title, priority}}
      ) => {
...
if (clientRes.type === 'error') {
  throw new Error(`Mutator failed on client: ${clientRes.error}`)
}
...
if (serverRes.type === 'error') {
  throw new Error(`Mutator failed on server: ${serverRes.error}`)
}
...
<Note heading="Returning data from mutators" slug="returning-data">
  There is not yet a way to return data from mutators in the success case. [Let us know](https://discord.rocicorp.dev/) if you need this.
</Note>
