...
┌──────────────────────────────────────────────────────────────────────────────┐
  ...
  ▼                     ▼                     ▼
  ┌──────────────────────────┐ ┌──────────────────────────┐ ┌──────────────────────────┐
  │ View‑Syncer Cluster A     │ │ View‑Syncer Cluster B     │ │ View‑Syncer Cluster N     │
  │ (ports configurable)      │ │ (ports configurable)      │ │ (ports configurable)      │
  │                           │ │                           │ │                           │
  │  ┌─────────────────────┐  │ │  ┌─────────────────────┐  │ │  ┌─────────────────────┐  │
  │  │ Syncer workers       │  │ │  │ Syncer workers       │  │ │  │ Syncer workers       │  │
  │  │ (ZERO_NUM_SYNC_)    │  │ │  │ (ZERO_NUM_SYNC_)    │  │ │  │ (ZERO_NUM_SYNC_*)    │  │
  │  │ - Serve WS clients   │  │ │  │ - Serve WS clients   │  │ │  │ - Serve WS clients   │  │
  │  │ - IVM + CVR updates  │  │ │  │ - IVM + CVR updates  │  │ │  │ - IVM + CVR updates  │  │
  │  │ - Forward mutations  │  │ │  │ - Forward mutations  │  │ │  │ - Forward mutations  │  │
  │  └─────────────────────┘  │ │  └─────────────────────┘  │ │  └─────────────────────┘  │
  │  │ Replicator           │  │ │  │ Replicator           │  │ │  │ Replicator           │  │
  │  │ - Subscribes to       │  │ │  │ - Subscribes to       │  │ │  │ - Subscribes to       │  │
  │  │   replication‑mgr WS │  │ │  │   replication‑mgr WS │  │ │  │   replication‑mgr WS │  │
  │  │ - Writes to SQLite   │  │ │  │ - Writes to SQLite   │  │ │  │ - Writes to SQLite   │  │
  │  └─────────────────────┘  │ │  └─────────────────────┘  │ │  └─────────────────────┘  │
  │            ▲               │ │            ▲               │ │            ▲               │
  │  ┌─────────────────────┐  │ │  ┌─────────────────────┐  │ │  ┌─────────────────────┐  │
  │  │ SQLite replica       │  │ │  │ SQLite replica       │  │ │  │ SQLite replica       │  │
  │  │ (local disk;         │  │ │  │ (local disk;         │  │ │  │ (local disk;         │  │
  │  │  restore from S3 if  │  │ │  │  restore from S3 if  │  │ │  │  restore from S3 if  │  │
  │  │  empty)              │  │ │  │  empty)              │  │ │  │  empty)              │  │
  │  └─────────────────────┘  │ │  └─────────────────────┘  │ │  └─────────────────────┘  │
  │                           │ │                           │ │                           │
  │  ┌─────────────────────┐  │ │  ┌─────────────────────┐  │ │  ┌─────────────────────┐  │
  │  │ CVR DB (local PG)    │  │ │  │ CVR DB (local PG)    │  │ │  │ CVR DB (local PG)    │  │
  │  └─────────────────────┘  │ │  └─────────────────────┘  │ │  └─────────────────────┘  │
  │  ┌─────────────────────┐  │ │  ┌─────────────────────┐  │ │  ┌─────────────────────┐  │
  ...
  ▼
  ┌──────────────────────────────────────────────────────────────────────────────┐
  │                    Replication‑Manager (single logical instance)              │
  │  (ports configurable)                                                         │
  │                                                                              │
  │  ┌─────────────────────────────────────────────────────────────────────────┐ │
  │  │ Change‑streamer                                                          │ │
  │  │ - Consumes upstream replication slot                                     │ │
  │  │ - Writes to Change DB (local PG)                                         │ │
  │  │ - Serves view‑syncers via WebSocket                                      │ │
  ...
  │                                                                              │
  │  ┌────────────────────────────────────┐   ┌──────────────────────────────┐   │
  │  │ Primary Postgres (RW)              │   │ API Servers                   │   │
  │  │ - app schemas (public.*)           │   │ - POST /api/mutate             │   │
  │  │ - upstream metadata schema         │◄──┤ - POST /api/query              │   │
  │  │   ({appID}_{shard}: clients,       │   │   (custom query transform)     │   │
