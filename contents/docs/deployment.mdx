...
shared replication slot, crashing the deployed instance. This includes monorepo
dev scripts (e.g. `turbo dev`) that start zero-cache as a workspace package —
if `ZERO_UPSTREAM_DB` points at a shared database, local zero-cache will evict
the deployed one. When the deployed instance is then restarted or redeployed, it
must perform a full re-sync of all replicated tables from Postgres, incurring
