...
While `connecting`, Zero repeatedly tries to connect to `zero-cache` for 5 minutes by default. This timeout can be configured with the `disconnectTimeoutMs` constructor parameter:
