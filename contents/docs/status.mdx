...
That said, we believe in building live. Zero can be, and is, used in production today for a certain applications that are an extremely good fit. Our path to beta involves expanding that set to an interesting size.
