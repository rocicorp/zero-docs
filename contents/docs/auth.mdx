...
Zero will forward this token to your [mutate](/docs/mutators) and [queries](/docs/queries) endpoints in an `Authorization: Bearer <token>` header, which you can use to authenticate the request as normal:
...
This will cause Zero to disconnect from `zero-cache` and the [connection status](/docs/connection) will change to `needs-auth`. You can then re-authenticate the user and call `zero.connection.connect()` to reconnect to `zero-cache`:
...
// or, if you are using a custom kvStore
const {dropped, errors} = await dropAllDatabases({ kvStore: customKvStore })
```
