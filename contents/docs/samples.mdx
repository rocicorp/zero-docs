...
<Note type="note">
  Zbugs is where we dogfood Zero. It uses tip-of-tree code and unreleased Zero features.

  Its primary purpose is to be a testbed and showcase for Zero, not to be an easy-to-follow example. For that, please see [ztunes](#ztunes) or other samples on this page.
</Note>
...
**Features:** Instant reads and writes, realtime updates, Github auth, write permissions, read permissions, mutators, complex filters, unread indicators, basic text search, emojis, short numeric bug IDs, notifications, and more.
