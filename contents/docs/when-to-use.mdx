...
Zero stores a replica of your database (at least the parts that can sync to client) in a SQLite database stored on the Zero server's disk. The ultimate limit of this database size is the size of the server's disk. But 1TB is a reasonable practical limit today.
