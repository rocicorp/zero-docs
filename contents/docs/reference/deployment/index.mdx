...
A single reasonably-sized `zero-cache` node can handle a fairly large number of users – hundreds to thousands depending on what they are doing. We run [zbugs](samples) on a single `m5.4xlarge` instance and have load tested it up to 4k concurrent users.
