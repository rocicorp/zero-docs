...
Zero does not write directly to your database.

Instead, just as in classic web apps, database writes happen _on your server_, with mutators.

A mutator is just a function with a database transaction and some arbitrary arguments passed in.

When a mutator is called, it executes instantly on the client, and updates the local store and query results. Then, it is synced in the background to the server.

The server can share the same mutator implementation with the client, or it can extend or override it completely. For example, you can write code that:
...
These writes to the database are then synced to other clients – even when they don't come from mutators.
...
<Note
  ...
  type="warning"
>
  We have deprecated the [old CRUD mutators](/docs/crud-mutators) and they will be removed in a future release.
</Note>
...
We hope to build [custom queries](https://bugs.rocicorp.dev/issue/3453) next – a read analog to mutators. If we succeed, Zero's permission system will go away completely 🤯.
