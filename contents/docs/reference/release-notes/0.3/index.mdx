...
- **Schema Migrations:** Zero now has first-class support for schema migration ([documentation](/docs/reference/schema-management)).
- **Write Permissions:** First-class write permissions based on ZQL ([documentation](/docs/reference/authentication-and-permissions)).
- **Date/Time related types:** Zero now natively supports the TIMESTAMP and DATE Postgres types ([sample app](https://github.com/rocicorp/hello-zero/blob/main/src/schema.ts), [documentation](/docs/reference/supported-postgres-features)).
- **SolidJS:** We now have first-class support for SolidJS ([documentation](/docs/reference/solidjs)).
...
- **New QuickStart App:** Entirely rewrote the [setup/sample flow](/docs/welcome/quickstart) to (a) make it much faster to get started playing with Zero, and (b) demonstrate more features.
...
- Started work to make [real docs](/docs/welcome/introduction). Not quite done yet.
