...
A single beefy `zero-cache` node can handle a fairly large number of users – hundreds to thousands of concurrents, depending on what they're doing.
...
Where should we deploy Zero to next? Let us know at [zero.rocicorp.dev/discord](https://zero.rocicorp.dev/discord)!
