...
The queries endpoint can return the exact same ZQL that ran on the client, or it could be different – it commonly includes additional clauses that enforce permissions.
...
You can use the [CRUD-style API](/docs/mutators#writing-data) with `tx.mutate(crud.<table>.<method>)`
...
You _must_ register your mutators with Zero before you can use them:
