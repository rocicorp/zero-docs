...
Most Postgres schema changes (e.g. `ALTER TABLE ...`, `ALTER PUBLICATION ...`, `ALTER SCHEMA ...`) are supported. There are two special cases which require additional steps:

### Adding a column with a non-constant DEFAULT value

An `ALTER TABLE ... ADD COLUMN ...` statement with a non-constant `DEFAULT` value is not supported. This includes expressions with parenthesis and special functions such as `CURRENT_TIME`, `CURRENT_DATE`, or `CURRENT_TIMESTAMP`.
(This is due to a [limitation of SQLite[(https://www.sqlite.org/lang_altertable.html#altertabaddcol)]).

However, the `DEFAULT` value of an _existing_ column can be changed to any value, including expressions. So the workaround is to:
...
### Adding tables or columns to a `PUBLICATION`

Postgres allows you to change the tables and/or columns that are published to the subscriber using an `ALTER PUBLICATION` statement. Zero will automatically adjust the tables and columns on the replica in response to changes in the publication, but it will not receive any of the existing data in those columns.

The workaround for this is to "touch" all of the rows in the added table to force postgres to stream the data to Zero:
