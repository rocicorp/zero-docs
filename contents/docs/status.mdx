...
* The API is in rapid development. We are deprecating the original "CRUD" mutators and ad-hoc queries and replacing them with the more flexible [custom mutators](/docs/custom-mutators) and [synced queries](/docs/synced-queries). Because documentation for both sets of features still exist, the docs are more complex than ideal.
...
* Zero plans single-table queries (choosing the right index, etc) by delegating to SQLite and this works well. Zero does not currently auto-plan joins. You can manually plan them using [the `flip` parameter](/docs/reading-data#join-flipping).
