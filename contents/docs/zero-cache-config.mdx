...
If true, zero-cache will forward cookies from the request to your mutate endpoint. This is useful for passing authentication cookies to the API server. If false, cookies are not forwarded.
...
Deprecated. Please use [Mutate Forward Cookies](#mutate-forward-cookies) instead.
...
If true, zero-cache will forward cookies from the request to your query endpoint. This is useful for passing authentication cookies to the API server. If false, cookies are not forwarded.
...
The URL of the API server to which zero-cache will send query requests.

`*` is allowed if you would like to allow the client to specify a subdomain to use.

For example: `*.example.com/api/query`

You can specify multiple URLs as well which the client can choose from.  
e.g., `["https://api1.example.com/query", "https://api2.example.com/query"]`
