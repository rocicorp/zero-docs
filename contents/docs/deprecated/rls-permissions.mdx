...
>
  ...
  [Permissions](/docs/auth#permissions) for more
  information.
</Note>
...
See the [SST Deployment Guide](/docs/self-host#guide-multi-node-on-sstaws) for more details.

## Rules

Each operation on a policy has a _ruleset_ containing zero or more _rules_.

A rule is just a TypeScript function that receives the logged in user's `AuthData` and generates a ZQL [where expression](/docs/queries#compound-filters). At least one rule in a ruleset must return a row for the operation to be allowed.
...
You can use the [`cmpLit` helper](/docs/queries#comparing-literal-values) to define permissions based on a field of the `authData` parameter:
