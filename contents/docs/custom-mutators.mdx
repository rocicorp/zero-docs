...
<Note type="danger" emoji="⚠️" heading="Not Returning a PushResponse" slug="not-returning-pushresponse">
  The server <em>must</em> return a `PushResponse` because the client expects
  one, even if it fails. If no such response is received, Zero will retry
  indefinitely. Since following mutations can rely on each other, they must be applied in order.
  So, if one mutation is not acknowledged, no mutation after that can be applied.

Throwing errors inside mutators in the `PushProcessor` will be handled for you.
However, doing things outside of it that return prematurely or throw errors, such as auth middleware or setup functions, will not.
