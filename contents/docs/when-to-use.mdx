...
Zero was designed from the ground up to be as close to a classic web app as possible. It can even interop with REST APIs using the same database.

### You use PostgreSQL

Other tools require you to use a non-standard backend database or data model. Zero works with PostgreSQL, and uses your existing schema.
