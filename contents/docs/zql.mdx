...
```tsx
const result = await zql.query.issue
  .where('id', 42)
  .one()
  .run()
if (!result) {
...
<Note type="note" slug="adding-operators">
  If you don’t see the comparison operator you need, let us
  know, many are easy to add.
</Note>
...
zql.query.issue.whereExists('comments', q =>
...
```ts
const complexQuery = zql.query.issue.related(
  'comments',
  q => q.related('author')
)
