...
| `zero_replica_backup_lag`                               | Gauge     | ms    | Time since last Litestream backup; expected to sawtooth according to the configured v3 or v5 backup interval |
...
`zero_replication_total_lag` and `zero_replication_last_total_lag` report the same latest measured round trip and do not grow when reports stop arriving. Use `zero_replication_lag_report_retries` to detect a stalled or missing report stream.
...
Serving-lag metrics include only client groups with at least one connected client and a validated background connection context. Retained groups without an eligible connection do not contribute lag. A client group also does not accumulate serving lag for a transaction that does not affect its queries.
