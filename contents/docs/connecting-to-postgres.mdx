...
It is easiest to connect zero-cache to PlanetScale with a role that has sufficient privileges to allow it to create its own publication.

If your Postgres role does not have permission to create publications, you can [create a publication manually](/docs/postgres-support#limiting-replication) and then specify that publication name in [App Publications](/docs/zero-cache-config#app-publications).
...
For the recommended approach to preview URLs, see [Preview Deployments](/docs/preview-deployments).
