...
You don't need to do anything specific to upgrade. Zero [will clamp your TTLs at `10m`](/docs/queries#setting-different-ttls) and print a warning.

But for best results, please review the [new TTL documentation](/docs/queries#ttls). In particular, see [how to set your TTLs](/docs/queries#setting-different-ttls) (TL;DR: You can often just remove them – [the defaults usually just work](/docs/queries#ttl-defaults)).

## Features

- Rework and simplify query TTLs ([docs](/docs/queries#ttls), [upgrading](#upgrading)).
...
- Add `useZeroOnline` to React and SolidJS bindings ([doc](/docs/connection#offline-ui)).
- Add `ttl` to `run()` ([PR](https://github.com/rocicorp/mono/pull/4603)).
- Allow client to specify `push.url` in the `Zero` constructor ([doc](/docs/mutators#server-setup)).
