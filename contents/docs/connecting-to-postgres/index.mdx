...
| Supabase, Neon, Render, Heroku    | 🤷‍♂️ Partial support with the `auto-reset` flag. See [Schema Changes](#schema-changes), below. See [Supabase notes](#supabase) below. |
...
You must [manually create a `PUBLICATION`](/docs/postgres-support#limiting-replication). `zero-cache` does not have permission to create a publication on its own.

### Supabase

In order to connect to Supabase you must use the "Direct Connection" style connection string, not the pooler:

<ImageLightbox src="/images/connecting-to-postgres/direct.png" alt="Screenshot of UI to get correct connection string" />
...
<ImageLightbox src="/images/connecting-to-postgres/ipv4.png" alt="Screenshot of where to assign ipv4 address" />
