...
<Note type="warning" heading="`zero-cache` is Currently Single-Node">
  This pains us as much as it does you.
...
A single reasonably-sized `zero-cache` node can handle a fairly large number of users – hundreds to thousands depending on what they are doing. We run [zbugs](samples) on a single `m5.4xlarge` instance and have load tested it up to 4k concurrent users clicking around madly.
...
<Note type="note">
  This is only needed temporarily, because Fly.io doesn't support TLS internally within its network and zero-cache requires it. This will be fixed soon.
</Note>
