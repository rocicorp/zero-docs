...
If the Zero client ever detects that its schema is incompatible with the server, it disconnecst and fires the [`onUpdateNeeded` event fires](#handling-old-clients) event. By default this reloads the app. This repeats (with backoff) until eventually the client gets a compatible schema.

If the API server ever detects that it has an incompatible schema, it will similarly fail with an error. The app will go into an [`error`](connection#error) state. You can reload the app when you're ready.

For local dev or staging, this is all fine and convenient.
...
<Note type="warning" heading="Test on staging first">
For production apps, we strongly recommend testing schema changes on a staging environment that has a production-like dataset before deploying to production.
</Note>
...
<Note type="warning" heading="Incorrect deployment order will cause downtime">
