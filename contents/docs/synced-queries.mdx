...
The context type passed can be anything, but it will most often be the currently logged in user, along with any other related state such as their role. It makes most sense to use the same context type for all your authenticated queries, so that you can treat queries generically [in your server handler](#server-setup).
