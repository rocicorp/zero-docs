...
>
  All users that have access to a browser profile have access to
  the same IndexedDB instances. There is nothing that Zero can do
  about this – users can just open the folder where the data is
  stored and look inside it.
</Note>
...
You can then access the context within your [queries](TODO) and [mutators](TODO) to implement permissions.
...
```ts
const allowedPosts = defineQuery(({ctx: {userID, role}}) => {
  if (role === 'admin') {
    return zql.post;
  }
  return zql.post.where('authorID', userID);
...
const updatePost = defineMutator(
  ...
  (tx, {ctx: {userID}, args: {id, content}}) => {
    const prev = await tx.run(
      zql.post
        .where('id', id)
        .related('sharedWith', q => q.where('userID', userID))
        .one(),
    );
    if (!prev) {
      return;
