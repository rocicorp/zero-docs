...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Scalar query typing        | **BREAKING.** Invalid or ineffective `scalar: true` calls that previously typechecked now fail compilation.                                                                    | Remove `scalar: true`, constrain every column of a primary key, or declare and constrain a real upstream unique key with `.unique(...)`. Only literal equality predicates prove uniqueness.                      |
| Flow-control configuration | **BREAKING.** `ZERO_CHANGE_STREAMER_FLOW_CONTROL_CONSENSUS_PADDING_SECONDS` is removed.                                                                                        | Configure `ZERO_CHANGE_STREAMER_FLOW_CONTROL_CONSENSUS_TIMEOUT_PROPORTION`; it is a ratio, not a unit conversion. The default is `2`. Set the new slow-subscriber grace period to `0` to disable forced resets.  |
...
- Type-safe scalar subqueries and `.unique(...)` schema declarations (#6307),
  with breaking migration guidance.
...
| `4b6205bbe` (#6307) | fix         | BREAKING  | Invalid `scalar: true` calls now fail TypeScript compilation; `.unique()` declares real upstream unique keys for typing.            | Propose Breaking Changes inclusion. Tests reject unpinned, non-unique, non-equality, and junction shapes.                                   |
...
| `06eee9cb4` (#6357) | performance | BREAKING  | Replaces fixed consensus padding with event-driven proportional flow control.                                                       | Propose public inclusion without a quantitative claim. The old setting is removed; default proportion is 2.                                 |
