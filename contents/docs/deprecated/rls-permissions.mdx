...
>
  ...
  [Permissions](/docs/auth#permissions) for more information.
</Note>
...
<Note type="note" slug="decompilation">
  The printed query can be different than the source ZQL string,
  because it is rebuilt from the query AST. But it should be
  logically equivalent to the query you wrote.
</Note>
...
<Note slug="ast-format">
  ...
  ASTs](./query-asts) to convert it to a more readable format.
</Note>
