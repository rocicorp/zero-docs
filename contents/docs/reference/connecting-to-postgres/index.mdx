...
In the future, Zero will work with many different backend databases. Today only Postgres is supported. Specifically, Zero requires Postgres v15.0 or higher, support for [logical replication](https://www.postgresql.org/docs/current/logical-replication.html).
...
Zero uses Postgres “[Event Triggers](https://www.postgresql.org/docs/current/sql-createeventtrigger.html)” to implement high-quality, efficient [schema management](https://www.postgresql.org/docs/current/sql-createeventtrigger.html). Event Triggers typically requires superuser access to Postgres.
...
To enable support for these databases, use the [`auto-reset`](https://www.notion.so/zero-cache-Config-1453bed89545802d9686c0ef84bf2a77?pvs=21) flag.
