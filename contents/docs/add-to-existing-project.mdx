...
```bash
export ZERO_UPSTREAM_DB="postgres://postgres:password@localhost:5432/postgres"
yarn zero-cache-dev # starts dev mode on port 4848
```
...
Zero doesn't allow clients to sync any arbitrary ZQL from the server – doing so would not make sense for security or performance reasons.
...
The queries endpoint can return the exact same ZQL that ran on the client, or it could be different – for example it might include additional clauses that enforce permissions.
