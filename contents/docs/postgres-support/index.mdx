...
`ALTER TABLE ... ADD COLUMN ...` with a non-constant `DEFAULT` value is not supported. This includes expressions with parenthesis and special functions such as `CURRENT_TIME`, `CURRENT_DATE`, or `CURRENT_TIMESTAMP`.
(This is due to a [limitation of SQLite[(https://www.sqlite.org/lang_altertable.html#altertabaddcol)]).

However, the `DEFAULT` value of an _existing_ column can be changed to any arbitrary expression. So the workaround is to:
