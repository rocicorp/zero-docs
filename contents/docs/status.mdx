...
That said, we believe in building live. Zero can be, and is, used in production today for a certain applications. Our path to beta involves expanding that set to an interesting size.

This page describes the current state of Zero at a high level. To understand whether Zero is currently a good fit for your application, please also see [When to Use Zero](./when-to-use).
</Note>
