...
Queries are the fundamental building block of Zero.

Despite their central role, superficially there's not much to them: A Zero query is basically just a function that returns an object describing the rows to sync.

## Defining Queries

Use `defineQuery` to create a query:
...
The return value of a query must be a [ZQL](zql) expression.
