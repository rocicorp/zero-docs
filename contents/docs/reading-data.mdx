...
It is inevetable that there will be cases where the requested data cannot be found. Because query results are never immediately populated, a simple existence check will result in flashing of the UI.

To prevent this from happening, the recommended way to handle missing data is to only display the not-found UI when the result type is `complete`. This way the 404 page is slow but pages with data are still just as fast.
