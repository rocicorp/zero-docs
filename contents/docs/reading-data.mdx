...
// 'foo' IS NOT null => true
// 'foo' IS NOT 'bar' => true
// 'foo' IS NOT 'foo' => false
...
This is no longer supported because for [synced queries](/docs/synced-queries) we needed to be able to return a ZQL query object from an async function. We didn't want this to implicitly invoke the query.
...
The main place this comes up is in [custom mutators](/docs/custom-mutators) where it is common to read some data using ZQL before writing a change.
</Note>
