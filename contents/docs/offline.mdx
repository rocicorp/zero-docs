...
<Note emoji="🤓" type="warning" heading="Well actually…">
  Offline writes *are* currently accepted by Zero, and sent when
  you come back online. But we plan to disable this soon.
</Note>
...
return <button disabled={!online}>Add to cart</button>;
```
