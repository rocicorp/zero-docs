...
<ImageLightbox
  src="/images/deployment/arch.png"
  caption="Zero deployment architecture"
/>
...
<Note heading="Custom mutators" slug="push-url">
  When using custom mutators, don't forget to set the push
  api url with `ZERO_PUSH_URL`
</Note>
...
<Note type="warning" slug="fly-app-names">
  **Note:** Fly.io requires app names to be unique across
  all Fly.io users. Change the `INITIALS` environment
  variable below to something unique.
</Note>
