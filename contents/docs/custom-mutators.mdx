---
title: Writing Data
description: Mutators are an extremely powerful way to write data with sync.
---

Data is written in Zero with mutators - they are very powerful, flexible, and expressive.

Mutators allow you to _create your own write operations_ using arbitrary code. This makes it possible to do things that are impossible or awkward with other sync engines.

For example, you can write code that:

- Performs arbitrary server-side validation
- Enforces fine-grained permissions
- Sends email notifications
- Queries LLMs
- Uses Yjs for collaborative editing
- … and much, _much_ more – mutators are just code, and they can do anything code can do!

Mutators participate fully in sync. They execute instantly on the local device, immediately updating all active queries. They are then synced in the background to the server and to other clients.
You can even share logic between the client and server mutators.

<Note
  slug="deprecating-crud"
  heading="Looking for the old mutators?"
  type="warning"
>
...
Mutators rely on a server in the Zero architecture.
...
Zero's mutators are based on [_server reconciliation_](https://www.gabrielgambetta.com/client-side-prediction-server-reconciliation.html) – a technique for robust sync that has been used by the video game industry for decades.
...
<Note emoji="🧑‍🏫" heading="A bit of history">
  ...
  Replicache's most popular features. Mutators bring this same power to
  Zero, but with a much better developer experience.
</Note>

A mutator is just a function that runs within a database transaction, and which can read and write to the database. Here's an example of a very simple mutator written in TypeScript:
...
Each mutator gets **two implementations**: one on the client and one on the server.
...
1. When you call a mutator on the client, Zero runs your client-side mutator immediately on the local device, updating all active queries instantly.
...
<Note type="note" heading="Run in Mutators">
...
You can use `run()` within mutators, but the `type` argument does nothing. In the future, passing `type` in this situation will throw an error.
...
`@rocicorp/zero/pg` includes a `ZQLDatabase` implementation of this interface backed by Postgres. The implementation allows the same mutator functions to run on client and server, by providing an implementation of the ZQL APIs that mutators run on the client.
...
Because mutators are just arbitrary TypeScript functions, there is no need for a special permissions system. Therefore, you do not need to use Zero's [write permissions](./permissions) (these will be deprecated in a future release).

<Note slug="read-permissions" emoji="💡" heading="You do still need *read* permissions">
  When using mutators you will have no [`insert`](permissions#insert-permissions), [`update`](permissions#update-permissions), or [`delete`](permissions#delete-permissions) permissions. You will still have [`select`](permissions#select-permissions) permissions, however.

We hope to build [named queries](https://bugs.rocicorp.dev/issue/3453) next – a read analog to mutators. If we succeed, Zero's permission system will go away completely 🤯.
...
There is no specific support for this in mutators, but since mutators are just code, it’s easy to do:
...
- Zbugs uses [mutators](https://github.com/rocicorp/mono/blob/a76c9a61670cc09e1a9fe7ab795749f3eef25577/apps/zbugs/shared/mutators.ts), [write permissions](https://github.com/rocicorp/mono/blob/a76c9a61670cc09e1a9fe7ab795749f3eef25577/apps/zbugs/shared/mutators.ts#L61), and [notifications](https://github.com/rocicorp/mono/blob/a76c9a61670cc09e1a9fe7ab795749f3eef25577/apps/zbugs/server/server-mutators.ts#L35).
- `hello-zero-solid` uses mutators for all [mutations](https://github.com/rocicorp/hello-zero-solid/blob/main/shared/mutators.ts) and permissions.
