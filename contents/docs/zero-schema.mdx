...
```bash
npm install -D @passionfroot/prisma-generator-zero
...
<Note emoji="🧑‍💻" heading="Both `drizzle-zero` and `prisma-zero` were community contributions" slug="contributing">
 We'd love more! See the source for [drizzle-zero](TODO) and [prismal-zero](TODO) as a guide, or reach out on [Discord](https://discord.rocicorp.dev/) with questions.
 </Note>
