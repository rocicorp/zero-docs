...
_Mutators_ are how you write data in Zero.

## Architecture

Zero relies on a special `/mutate` endpoint on your server to implement writes.

This keeps your server in control and allows you to implement arbitrary server-side logic for each mutation, including permissions, validation, and more.

![diagram](/images/mutators/overview.png)

Zero provides utilities in `@rocicorp/zero` that make it really easy implement `/mutate` in TypeScript. But you can also implement it yourself if you want. As long as your endpoint fulfills the push protocol, `zero-cache` doesn't care. You can even write it in a different programming language.
...
- `https://mybranch-aaa.preview.myapp.com/push` ✅
- `https://mybranch-bbb.preview.myapp.com/push` ✅

But rejects URLs like:
- `https://preview.myapp.com/push` ❌ (missing subdomain)
- `https://malicious.com/push` ❌ (different domain)
- `https://mybranch-123.preview.myapp.com/push/extra` ❌ (extra path)
- `https://mybranch-123.preview.myapp.com/other` ❌ (different path)

<Note emoji="🥇" heading="Pro Tip (tm)">
