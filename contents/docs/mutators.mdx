...
const initStats = defineMutator(async ({tx}) => {
  await tx.mutate(
    crud.stats.insert({
      id: 'global',
      issueCount: 0,
...
>
  ...
  client, within milliseconds. The reason they are marked `async`
  is because on the server, reading from the `tx` object goes
  over the network to Postgres. Also, in edge cases on the
  client, operations can go to local storage (IndexedDB or
  SQLite).
</Note>
...
You can optionally pass a _validator_ to `defineMutator` as the first argument:
...
<Note heading="Using a different bindings library">
  Zero includes several built-in database adapters. You can also
  easily create your own. See [ZQL on the Server](server-zql) for
  more information.
</Note>
...
if (clientRes.type === 'error') {
  throw new Error(
    `Mutator failed on client: ${clientRes.error}`,
  );
}
...
if (serverRes.type === 'error') {
  throw new Error(
    `Mutator failed on server: ${serverRes.error}`,
  );
}
...
>
  ...
  success case. [Let us know](https://discord.rocicorp.dev/) if
  you need this.
</Note>
...
export const mutators = {
  issue: {
    update: defineMutator(
      ...
      async (tx, {id, title}) => {
        await tx.mutate(crud.issue.update({id, title}));
