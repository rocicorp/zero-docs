...
- **Compound FKs.** Zero already supported compound _primary_ keys, but now it also supports compound _foreign_ keys ([docs](../schema#relationships-and-compound-keys)).
- **Schema DX**:
...
<Note type="note">
  ...
  server without needing a TS toolchain there. Also so that we
  can run a SaaS in the future without needing to run user code.
</Note>
