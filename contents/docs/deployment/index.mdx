...
### Add Permission Deployer

Copy the `functions` directory from [hello-zero](https://github.com/rocicorp/hello-zero/tree/main/functions) into your project. Adjust `tsconfig.json` as necessary for your project.

<Note type="warning">
The version of `@rocicorp/zero` in `functions/package.json` should match the version of `@rocicorp/zero` in your app.

Sorry, we're working on making this less annoying.
</Note>

This small function runs inside your VPC to [deploy permission changes](./permissions#permission-deployment). There are other ways to do this, but this way is nice because:

1. Permission changes are automatically deployed as part of deploying your app.
2. Your CI/CD pipeline doesn't need direct access to your database.
