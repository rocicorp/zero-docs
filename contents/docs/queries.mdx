...
export const queries = defineQueries({
  posts: {
    get: defineQuery(z.string(), id => zql.post.where('id', id)),
...
>
  ...
  query (i.e., `posts.get`, `users.byRole`), it should only be
  used once at the top level of your `queries.ts` file.
</Note>
...
<Note emoji="🥇" heading="Pro Tip (tm)">
  Because URLPattern is a web standard, you can test them right
  in your browser:
  <img
    alt="URL Pattern"
...
// With run():
const user = await zero.run(queries.user.byID('id', userId), {
  ttl: '5m',
});

// With materialize():
const view = zero.materialize(queries.user.byID('id', userId), {
  ttl: '5m',
});
```
