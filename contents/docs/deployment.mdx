...
Zero-cache is backed by a SQLite replica of your database. The SQLite replica uses upstream Postgres as the source of truth. If the SQLite replica is somehow corrupted or the litestream backup disappears, the replication-manager can resync its source replica from it.
...
1. Run the database migrations (using the [expand/migrate/contract pattern](/docs/schema#schema-change-process)) and wait for replication to catch up.
