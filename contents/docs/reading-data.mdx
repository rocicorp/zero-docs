...
Zero implements `exists` using an [inner join](https://en.wikipedia.org/wiki/Join_(SQL)) internally. As in any database, the order the tables are joined in dramatically affects performance.

Zero doesn't yet have a query planner that can automatically pick the best join order. But you can control the join order manually using the `flip:true` option to `whereExists`:
...
<Note type="note" heading="When to use join flipping">
  You should use `flip:true` when the query passed to `exists()` is expected to return a match only for a small subset of the rows in the parent table.

  By default, Zero implements `exists` by looping through all the rows of the parent query and for each row, checking if there is a matching row in the child query. This strategy works well if the child filter is expected to match a large fraction of the parent rows. For example in [zbugs](samples#zbugs), most users can access most bugs, so Zero's default strategy works well.

  But if you know that the child query will match a small fraction of rows in parent query, use the `flip` to tell Zero to loop through the child query first.
</Note>
