...
[Mutators](./writing-data) use ZQL on the server as an implementation detail, but you can also use ZQL on the server directly.
...
If ZQL does not have the featuers you need, you can use `tx.dbTransaction` to [drop down to raw SQL](./writing-data#dropping-down-to-raw-sql).
...
It is also possible to use ORMs like Drizzle. Wrap the drizzle transaction and now you can access drizzle's transaction within server-side mutators.
