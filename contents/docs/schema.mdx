...
## Generating from Drizzle

If you use Drizzle ORM, you can generate `schema.ts` with [`drizzle-zero`](https://www.npmjs.com/package/drizzle-zero):
...
## Generating from Prisma

If you use Prisma, you can generate `schema.ts` with [`prisma-zero`](https://www.npmjs.com/package/prisma-zero):
...
<Note
  emoji="🧑‍💻"
  heading="Both `drizzle-zero` and `prisma-zero` were community contributions"
  slug="contributing"
>
  We'd love more! See the source for
  [drizzle-zero](https://github.com/rocicorp/drizzle-zero) and
  [prisma-zero](https://github.com/rocicorp/prisma-zero) as a
  guide, or reach out on [Discord](https://discord.rocicorp.dev/)
  with questions.
</Note>
...
<Note type="warning" slug="postgres-special-cases">
  ...
  See [Schema Changes](/docs/postgres-support#schema-changes) for
  details.
</Note>
