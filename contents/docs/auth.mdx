...
>
  ...
  `clientGroupID` during connection. Passing the
  server-verified `userID` to `handleMutateRequest` and
  `handleQueryRequest` lets Zero enforce that only tabs
  belonging to the original user can join.
</Note>
