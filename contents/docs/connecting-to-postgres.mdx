...
App roles can't create [event triggers](#event-triggers), so schema changes will fall back to full resets unless you call [`zero_0.update_schemas()`](#schema-change-hooks) after each DDL statement.
...
For providers that don't support or allow [event triggers](#event-triggers), Zero provides a manual hook you can call after a schema change to avoid a full reset. Wrap the DDL and the `update_schemas()` call in a single transaction so writes against the new schema don't slip into the WAL before `zero-cache` learns about the change:
...
The function lives in the `{app-id}_{shard-num}` schema. With the default [app id](/docs/zero-cache-config#app-id) of `zero` and the default shard `0`, it's `zero_0.update_schemas()`.

It works for any DDL statement (`CREATE`, `ALTER`, `DROP`, `ALTER PUBLICATION`, etc.). Because it diffs the current schema against the last snapshot, you can call it once after a batch of statements rather than after each one.

<Note type="note" heading="COMMENT ON PUBLICATION workaround">
  Another way to tell Zero about a publication change is to follow the `ALTER PUBLICATION` with a `COMMENT ON PUBLICATION` statement:
...
This was the original workaround for publication changes. It is still supported, but was replaced by `update_schemas()` because the latter is explicit and applies to any DDL, not just publication changes.
