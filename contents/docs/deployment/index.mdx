...
This pains us as much as it does you, because `zero-cache` is designed to be scalable – it's just not quite wired up yet.

We will address this in the near future with features like rolling updates, sharding different subsets of data into different instances, and geographic replication.

See [Roadmap](roadmap) for more information.

</Note>

A single reasonably-sized `zero-cache` node can handle a fairly large number of users – hundreds to thousands of concurrents depending on what they are doing. We run [zbugs](samples) on a single `m5.4xlarge` instance and have load tested it up to 4k concurrent users clicking around madly.

But it's not infinitely scalable, and updating `zero-cache` will take down your service for a minute or so while the Docker container restarts. We'll have this fixed soon.
...
Where should we deploy Zero next?? Let us know at [discord.zero.rocicorp.dev](https://discord.zero.rocicorp.dev)!
