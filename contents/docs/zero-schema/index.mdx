...
- The Zero schema includes _permission rules_ that control access to the database.
...
Use `table` to define each table in your Zero schema:
...
Columns can have the types `boolean`, `number`, `string`, `null`, `json`. See [Column Types](/docs/postgres-support#column-types) for how database types are mapped to these types. These types can also take a generic TypeScript parameter if you'd like to further narrow them to something more specific. E.g., `number<Timestamp>()` or `json<{registrar: 'github' | 'google', id: string}>{}`.
...
```tsx
import {enumeration, table, string} from '@rocicorp/zero';
...
Specify additional columns to the `primaryKey` function for compound primary keys:
...
Use the `relationships` function to define relationships between tables:

```ts
const messageSchema = table('message')
  .columns({
    id: string(),
    senderID: string(),
  })
...
const messageRelationships = relationships(messageSchema, ({one}) => ({
  sender: one({
...
const issueRelationships = relationships(issueSchema, ({many}) => ({
  labels: many(
    {
      sourceField: ['id'],
...
export const schema = createSchema(1, {
  tables: [userSchema, mediumSchema, messageSchema],
  relationships: [userRelationships, mediumRelationships, messageRelationships],
});
```

The first parameter is the schema `version` and is used as part of [schema migration](/docs/migrations).
