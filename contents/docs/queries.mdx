...
Queries are how you read and sync data with Zero, and are a core primitive used to build Zero apps.
...
<Note emoji="🧑‍💻" heading="Queries are code">
...
`withValidation` uses the `validator` function you provided when defining the query to validate the arguments passed to the query. If validation fails, the invocation throws and an error is returned to the client.
...
By default, Zero sends queries to the URL specified in the `ZERO_QUERIES_URL` parameter.
...
- `https://mybranch-aaa.preview.myapp.com/queries` ✅
- `https://mybranch-bbb.preview.myapp.com/queries` ✅

But rejects URLs like:
- `https://preview.myapp.com/queries` ❌ (missing subdomain)
- `https://malicious.com/queries` ❌ (different domain)
- `https://mybranch-123.preview.myapp.com/queries/extra` ❌ (extra path)
