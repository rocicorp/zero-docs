...
You can read data within a mutator using [ZQL](./reading-data):

```ts
const updateIssue = defineMutatorWithContext<MyZeroContext>()(
  tx,
  z.object({
    id: z.string(),
  ...
  async (tx) => {
    const issue = await tx.local(zql.issue.where('id', id).one());
    if (!issue) {
...
When you call `z.query.foo.run()` outside of a mutator, there is a `type` parameter that can be used to wait for server results.

There's no equivalent of this inside mutators, because waiting for server results in mutators makes no sense – it would defeat the purpose of running optimistically to begin with.
