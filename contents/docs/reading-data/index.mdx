...
This is kind of crazy, but works surprisingly well for zbugs. Anyway, we're working on cleaning it up.

We will be adding an LRU cache for queries, so that they aren't removed from the server when the client stops listening. This means that if you do a query, then the same query a little later, it will typically resolve instantly automatically.
...
<Note type="note">
The `run()` method does not yet expose [`resultType`](#completeness) in any way, so you can't use this method to wait for an authoritative result from the server. See [bug 3243](https://bugs.rocicorp.dev/issue/3243) for a workaround.
</Note>
