...
- Improved React lifecycle management with `ZeroProvider` ([docs](/docs/react#setup), [PR](https://github.com/rocicorp/mono/pull/4527)).
- Expose `Zero` instances automatically at `__zero` ([docs](/docs/debug/inspector#accessing-the-inspector), [PR](https://github.com/rocicorp/mono/pull/4526)).
...
- Using `order by` and `limit` in many-to-many relationships now throws an error. It didn't work before but did the wrong thing silently. Now it throws a runtime error. See [docs](/docs/zql#order-and-limit-not-supported-in-junction-relationships), [bug](https://bugs.rocicorp.dev/issue/3527).
