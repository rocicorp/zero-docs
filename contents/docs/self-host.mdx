...
The examples below use Docker Hub, but the Zero container image is available from:

- Docker Hub: `rocicorp/zero:{version}`
- GHCR: `ghcr.io/rocicorp/zero:{version}`
...
<Note
  type="danger"
  heading="Do not expose replication-manager to the internet"
  slug="private-replication-manager"
>
...
<Note
  type="note"
  emoji="💡"
  heading="Separate Zero changes from schema changes"
  slug="separate-zero-and-schema-changes"
>
