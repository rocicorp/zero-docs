...
You do not need to define the Zero schema entirely by hand. [Community-contributed converters](./community#database-tools) exist for Prisma and Drizzle that generate the tables and relationships. It is good to know how to write the schema by hand, however, for debugging and conceptual understanding.

This page describes using the schema to define your tables, columns, and relationships. For information on permissions, see [Permissions](/docs/permissions). For information on migration see [Schema Migration](/docs/migrations).
