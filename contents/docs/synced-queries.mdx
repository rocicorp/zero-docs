...
If the query endpoint returns an HTTP or network error, it will be reported to the client with using the `type` and `error` fields on the query details object:

```ts
const [posts, details] = useQuery(myPosts('user123'));
...
In React and SolidJS, you can re-run the failed query using `refetch`:

```ts
const [posts, details, { refetch }] = useQuery(myPosts('user123'));
if (details.type === 'error') {
  return (
    <div>
      Error loading posts: {details.error.message}
...
When using the low-level API, just call `materialize()` again.

### Custom Get Queries URL

By default, synced queries use the URL specified in the `ZERO_GET_QUERIES_URL` parameter.
...
The URLs listed in `ZERO_GET_QUERIES_URL` can contain `*` wildcards. See the [URL Pattern Syntax](./custom-mutators#mutate-url-patterns) section in the Custom Mutators documentation for details. It works the same way for synced queries.
