...
<details>
<summary id="not-npm">Using `pnpm` or `bun`?</summary>
...
By default, all permissions are denied. Allow access to tables by [adding permissions](permissions).

For example, to allow all users to read and write to the `message` table, add the following:
...
#### Add the ZeroProvider

First, wrap your app with a `ZeroProvider` to make Zero available throughout your component tree.
...
export function App() {
  const authData = useAuth(); // replace with your auth logic

  const props = useMemo(() => {
    return {
      schema,
      server: 'http://localhost:4848', // replace with an env var
...
Now you can use the `useZero` hook in any component within the provider:
...
function IssueList() {
  ...
  const issueQuery = z.query.issue.limit(100);
...
Wrap your app with a SolidJS `ZeroProvider` to make Zero available throughout your component tree.
...
Use the `useZero` hook within any component inside the provider:
...
function IssueList() {
  const z = useZero();
  const [issues] = useQuery(() => z.query.issue.limit(100));
...
Zero supports both simple and advanced data mutations. The mutator API on the client looks something like this:
