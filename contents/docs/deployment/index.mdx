...
A running `zero-cache` is composed of a single `replication-manager` node and multiple `view-syncers` nodes. It also depends on Postgres, S3, and attached SSD storage.

**Upstream:** Your application's Postgres database.

**Change DB:** A Postgres DB uses by Zero to store a recent subset of the Postgres replication log.
...
**View Syncers:** Handle WebSocket connections from clients and run ZQL queries. Updates CVR DB with the latest state of each client as queries run. Uses CVR DB on client connection to compute initial diffto catch clients up.
...
When run with multiple View Syncer nodes, `zero-cache` supports rolling, downtime-free updates. A new Replication Manager starts up and connections drain from the old View Syncers. When all old View Syncers are gone, the old Replication Manager is shut down.
