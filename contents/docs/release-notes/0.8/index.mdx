...
See the changes to [hello-zero](TODO) or [hello-zero-solid](TODO) for example updates.

## Features

- **Schema Autobuild**. There's now a `zero-cache-dev` script that automatically rebuilds the schema and restart `zero-cache` when you change `schema.ts`. ([docs](../zero-schema#building-the-zero-schema))
...
- **Permission Helpers**. `ANYONE_CAN` and `NOBODY_CAN` helpers were added to make these cases more readable. ([docs](../permissions#helpers))
- **Multitenant Support**. A single `zero-cache` can now front separate Postgres databases. This is useful for customers that have one "dev" database in production per-developer. ([docs](../zero-cache-config#multitenant))
